...
public class ServicioCarritoMock implements IServicioCarritoMockRemote, IServicioCarritoMockLocal {
     ...
     */
    // Add this method to your IServicioCarritoMockRemote interface
    @Override
    public void removerItem(Mueble mueble, boolean removerCero) {
     ...
     @Override
    public void actualizarItem(Mueble mueble, int cantidad) {
        Mueble item;
        for (int i = 0, max = inventario.size(); i < max; i++) {
            item = (Mueble) inventario.get(i);
            if (item.getReferencia() == mueble.getReferencia()) {
    ...
    /**
     * Actualiza la cantidad de un mueble en el carrito de compras.
     *
     * @param referencia Referencia del mueble a actualizar
     * @param nuevaCantidad Nueva cantidad del mueble
     * @throws OperacionInvalidaException Si el mueble no se encuentra en el
     * carrito
     */
